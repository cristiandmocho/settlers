...
export default class ChestInterface extends EventEmmiter {
  constructor(bot) {
    super();
    this.bot = bot;
  }

  Deposit(chest, itemName, amount) {
    chest.on("open", () => {
      ...
      if (item) {
        chest.deposit(item.type, null, amount, (err) => {
          if (err) {
            ...
            this.emit("error", { message: err.stack });
          } else {
            console.log(`Depositou ${amount} ${itemName}`);
            this.emit("deposit", { itemName, amount });
          }
        });
      } else {
        const message = `No ${itemName} in inventory`;
        console.log(message);
  ...
  Withdraw(chest, itemName, amount) {
    chest.on("open", () => {
      const item = chest
        .containerItems()
        .find((item) => item.name === itemName);
      ...
      if (item) {
        chest.withdraw(item.type, null, amount, (err) => {
          if (err) {
            ...
            console.log(`Retirou ${amount} ${itemName}`);
            this.emit("withdraw", { itemName, amount });
          }
        });
      } else {
        const message = `No ${itemName} in chest`;
        console.log(message);
        this.emit("error", { message });
      }
    });
  ...
  FindChest(maxDistance = 6) {
    const chest = this.bot.openChest(
      this.bot.findBlock({
...
async function watchChest(minecart, blocks = []) {
  ...
  if (minecart) {
    chestToOpen = Object.keys(bot.entities)
      ...
      .find(
        (e) =>
          e.entityType === bot.registry.entitiesByName.chest_minecart &&
  ...
  chest.on("updateSlot", (slot, oldItem, newItem) => {
    bot.chat(
      `chest update: ${itemToString(oldItem)} -> ${itemToString(
        newItem
      )} (slot: ${slot})`
    );
  });
...
async function watchFurnace() {
  ...
  furnace.on("updateSlot", (slot, oldItem, newItem) => {
    bot.chat(
      `furnace update: ${itemToString(oldItem)} -> ${itemToString(
        newItem
      )} (slot: ${slot})`
    );
  });
...
async function watchEnchantmentTable() {
  ...
  table.on("updateSlot", (slot, oldItem, newItem) => {
    bot.chat(
      `enchantment table update: ${itemToString(oldItem)} -> ${itemToString(
        newItem
      )} (slot: ${slot})`
    );
  });
  ...
  table.on("ready", () => {
    bot.chat(
      `ready to enchant. choices are ${table.enchantments
        .map((o) => o.level)
        .join(", ")}`
    );
  });
  ...
  function onChat(username, message) {
    ...
    async function addLapis() {
      const item = itemByType(
        table.window.items(),
        ["dye", "purple_dye", "lapis_lazuli"]
          .filter((name) => bot.registry.itemByName[name] !== undefined)
